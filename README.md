# Hi, I'm Luth (Luther History)

Self-taught developer focused on C++ and systems programming.  
I learn mainly by building projects to understand how software works internally — from algorithms to GUI applications and system interaction.

---

## About Me

- Main language: C++  
- Linux user interested in low-level concepts  
- Chess player building personal software projects  
- Prefer understanding fundamentals rather than copying solutions  

---

## Current Focus

- Game and engine architecture  
- Software design and clean code  
- Strengthening computer science fundamentals  

---

## Projects

| Project Name | Status |
|--------------|--------|
| Awesome Chess | ![Awesome Chess](https://img.shields.io/badge/Active-70%25-blue) |
| Awesome Engine | ![Awesome Engine](https://img.shields.io/badge/Planned-0%25-lightgrey) |
| Jujutsu Kaisen Fan-Made Game | ![Jujutsu Kaisen Fan-Made Game](https://img.shields.io/badge/Planned-0%25-lightgrey) |
| Level Up | ![Level Up](https://img.shields.io/badge/Planned-0%25-lightgrey) |
| mini-Terminal | ![Mini-Terminal](https://img.shields.io/badge/Planned-0%25-lightgrey) |
| Red Panda Debugger | ![Red Panda Debugger](https://img.shields.io/badge/Planned-0%25-lightgrey) |

---

## Tech Stack

**Primary**  
- C++

**Basic Knowledge / Exploring**  
- C  
- Rust  

---

## Skills

| Area | Status |
|------|--------|
| C++ | Intermediate (project-focused) |
| Rust | Learning |
| Computer Science Fundamentals | In progress |

---

## Philosophy

Learn deeply. Build consistently.  

---

## Links

[Telegram](https://t.me/code_by_luth)  
[YouTube](https://www.youtube.com/@code_by_luth)
